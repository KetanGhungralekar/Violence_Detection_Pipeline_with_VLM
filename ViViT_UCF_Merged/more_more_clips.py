...
def process_video_row(row_index: int, row: pd.Series, source_dir: Path, output_dir: Path):
    ...
    # 1. Check if the original CSV range is shorter than our max clip length.
    #    If so, save this original short clip *in addition* to the augmented clips.
    if range_duration_s < MAX_CLIP_LENGTH_S:
        print(f"    Range ({range_duration_s:.2f}s) is shorter than max length ({MAX_CLIP_LENGTH_S}s).")
        print(f"    Saving the original short clip first.")
        # Save the original short clip with a special name
    ...
    # 2. Now, *always* run the augmentation loop.
    #    This loop creates 10s clips, starting as long as the start time
    #    is within the CSV [start_s, end_s] range.
    ...
    if range_duration_s < MAX_CLIP_LENGTH_S:
        ...
        print(f"    -> Generated {clip_counter} augmented {MAX_CLIP_LENGTH_S}s clips with a step of {step_size_d:.2f}s")
