...
import numpy as np
import pandas as pd
from vivit_multitask import load_video
...
try:
    ...
    from vivit_multitask import ViViT
...
class VideoDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        label_binary = torch.tensor(self.labels[idx][0], dtype=torch.long)
        label_fine = torch.tensor(self.labels[idx][1], dtype=torch.long)
        return video, label_binary, label_fine
...
class GPUVideoDataset(Dataset):
    ...
    """
    def __init__(self, video_paths, labels, num_frames=16, image_size=224, device='cuda', preload=True):
        self.video_paths = video_paths
        self.labels = labels
        self.num_frames = num_frames
        self.image_size = image_size
        self.device = device
        self.videos_gpu = []
        self.labels_binary_gpu = []
        self.labels_fine_gpu = []
    ...
    def _preload_to_gpu(self):
        ...
        for idx, video_path in enumerate(tqdm(self.video_paths, desc="Loading videos to GPU")):
            try:
                ...
                label_binary_gpu = torch.tensor(self.labels[idx][0], dtype=torch.long, device=self.device)
                label_fine_gpu = torch.tensor(self.labels[idx][1], dtype=torch.long, device=self.device)
                
                self.videos_gpu.append(video_gpu)
                self.labels_binary_gpu.append(label_binary_gpu)
                self.labels_fine_gpu.append(label_fine_gpu)
    ...
    def __getitem__(self, idx):
        return self.videos_gpu[idx], self.labels_binary_gpu[idx], self.labels_fine_gpu[idx]

# ----------------------- Data Preparation -----------------------
def load_data(data_root="../../UCF_MERGED_NORM/"):
    """
    Load data from a folder containing class subdirectories (with optional nested subdirectories).
    All videos found recursively within each class directory are treated as belonging to that class.
    ...
    for class_name in data_subset:
        ...
        # Recursively find all video files in this class directory and its subdirectories
        import glob
        video_files = []
        for ext in ['*.mp4', '*.avi', '*.mov', '*.mkv']:
            video_files.extend(glob.glob(os.path.join(class_dir, '**', ext), recursive=True))
        
        logger.info(f"Found {len(video_files)} videos in class '{class_name}'")
        
        for video_path in video_files:
            if os.path.isfile(video_path):
                all_videos.append(video_path)
                binary_label = 0 if 'Normal' in class_name else 1
                all_labels.append((binary_label, class_to_idx[class_name]))
            else:
...
def train_epoch(model, train_loader, criterion, optimizer, scaler, device, epoch, total_epochs, class_names, save_dir):
    ...
    for batch_idx, (videos_batch, binary_labels_batch, fine_labels_batch) in enumerate(tqdm(train_loader, desc=f"Epoch {epoch+1}/{total_epochs}")):
        ...
        if not binary_labels_batch.is_cuda:
            binary_labels_batch = binary_labels_batch.to(device, non_blocking=PIN_MEMORY)
        if not fine_labels_batch.is_cuda:
            fine_labels_batch = fine_labels_batch.to(device, non_blocking=PIN_MEMORY)
        
        if MIXED_PRECISION:
            with autocast('cuda'):
                binary_output, fine_output = model(videos_batch)
                loss_binary = criterion(binary_output, binary_labels_batch)
                loss_fine = criterion(fine_output, fine_labels_batch)
        ...
        batch_size = fine_labels_batch.size(0)
        epoch_loss += loss.item() * batch_size
        total_count += batch_size
        
        preds = fine_output.argmax(dim=1).detach().cpu().numpy()
        labels_np = fine_labels_batch.detach().cpu().numpy()
...
def validate(model, val_loader, criterion, device, class_names, save_dir, epoch):
    ...
    with torch.no_grad():
        for videos_batch, binary_labels_batch, fine_labels_batch in tqdm(val_loader, desc="Validation"):
            # Only transfer to device if not already on GPU (for non-GPU preloaded datasets)
            if not videos_batch.is_cuda:
                videos_batch = videos_batch.to(device, non_blocking=PIN_MEMORY)
            if not binary_labels_batch.is_cuda:
            ...
            if MIXED_PRECISION:
                with autocast('cuda'): # added cuda 10th april
                    binary_output, fine_output = model(videos_batch)
                    loss = criterion(binary_output, binary_labels_batch) + criterion(fine_output, fine_labels_batch)
            else:
                binary_output, fine_output = model(videos_batch)
                loss = criterion(binary_output, binary_labels_batch) + criterion(fine_output, fine_labels_batch)
            ...
            preds = fine_output.argmax(dim=1).cpu().numpy()
            val_labels.extend(fine_labels_batch.cpu().numpy())
...
def evaluate_test(model, test_loader, device, class_names, save_dir):
    ...
    with torch.no_grad():
        for videos_batch, binary_labels_batch, fine_labels_batch in tqdm(test_loader, desc="Testing"):
            # Only transfer to device if not already on GPU (for non-GPU preloaded datasets)
            if not videos_batch.is_cuda:
                videos_batch = videos_batch.to(device, non_blocking=PIN_MEMORY)
            if not binary_labels_batch.is_cuda:
            ...
            all_labels.extend(fine_labels_batch.cpu().numpy())
...
def main():
    ...
    if not TRANSFORMERS_AVAILABLE:
        ...
        num_fine_grained_classes = len(data_subset)
        model = ViViT(image_size=224, patch_size=16, num_fine_grained_classes=num_fine_grained_classes, num_frames=16,
        ...
        model = load_pretrained_weights(model, pretrained_weights_path, num_classes_pretrained=101, num_classes_current=num_fine_grained_classes)
