...
# ----------------------- Data Preparation -----------------------
def load_data(data_root="../../UCF_MERGED/"):
    """
...
def main():
    ...
    if torch.cuda.is_available():
        ...
        # Limit GPU memory usage to ~8GB to allow running another process
        torch.cuda.set_per_process_memory_fraction(0.18)  # ~8GB out of 49GB
        torch.cuda.empty_cache()
        logger.info("GPU memory limited to ~8GB per process")
    else:
    ...
    if TRANSFORMERS_AVAILABLE:
        ...
        try:
            ...
            # Interpolate position embeddings from 32 frames to 16 frames
            old_num_frames = 32
            new_num_frames = 16
            tubelet_temporal = config.tubelet_size[0]  # 2
            patch_spatial = config.tubelet_size[1]  # 16
            image_size = config.image_size  # 224
            spatial_patches = (image_size // patch_spatial) ** 2  # 196
            old_temporal_patches = old_num_frames // tubelet_temporal  # 16
            new_temporal_patches = new_num_frames // tubelet_temporal  # 8
            old_num_patches = old_temporal_patches * spatial_patches + 1  # 3137
            new_num_patches = new_temporal_patches * spatial_patches + 1  # 1569
            
            pos_emb = model.vivit.embeddings.position_embeddings  # (1, 3137, 768)
            pos_emb_interp = F.interpolate(pos_emb.transpose(1, 2), size=new_num_patches, mode='linear', align_corners=False).transpose(1, 2)
            model.vivit.embeddings.position_embeddings = nn.Parameter(pos_emb_interp)
            
            # Update config
            config.num_frames = new_num_frames
