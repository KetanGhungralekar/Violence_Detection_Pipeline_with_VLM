...
class Transformer(nn.Module):
    ...
    def forward(self, x, return_attention=False):
        ...
        for attn, ff in self.layers:
            attn_output, weights = attn(x, return_attention=True)
            x = attn_output + x
            x = ff(x) + x
            if return_attention:
...
class ViViT(nn.Module):
    def __init__(self, image_size, patch_size, num_fine_grained_classes, num_frames, dim = 192, depth = 4, heads = 3, pool = 'cls', in_channels = 3, dim_head = 64, dropout = 0.1,
        ...
        # Binary classification head (normal/abnormal)
        ...
        self.fine_grained_head = nn.Sequential(
            nn.LayerNorm(dim),
            nn.Dropout(dropout),
            nn.Linear(dim, num_fine_grained_classes)
        )
    ...
    def forward(self, x, return_attention=False):
            ...
            if return_attention:
                x, space_attn_weights = self.space_transformer(x, return_attention=True)
            else:
            ...
            if return_attention:
                x, temporal_attn_weights = self.temporal_transformer(x, return_attention=True)
            else:
            ...
            fine_grained_output = self.fine_grained_head(x)

            if return_attention:
                return (binary_output, fine_grained_output), (space_attn_weights, temporal_attn_weights)
            return binary_output, fine_grained_output
...
def load_video(video_path, num_frames=16, image_size=224):
    ...
    transform = transforms.Compose([
        transforms.ToTensor(),
        transforms.Normalize(mean=[0.3763090481649224, 0.36528133079917907, 0.3509803885002057], std=[0.2708064877155682, 0.266584700847205, 0.26854724531039814])
    ])
...
def load_video_new(video_path, num_frames=16, image_size=224):
    try:
        cap = cv2.VideoCapture(video_path)
        if not cap.isOpened():
            print(f"Error: Unable to open video {video_path}. Returning zeros.")
    ...
    while True:
        ...
        try:
            ...
            print(f"Error processing frame from video {video_path}: {e}")
            continue
    
    cap.release()

    # Define transformation pipeline (only ToTensor, let the augmentation pipeline handle normalization)
    transform = transforms.Compose([
        transforms.ToTensor(),
    ])
    ...
    # Convert all frames to tensor
    frames = torch.stack([transform(frame) for frame in frames])  # Shape: [T, C, H, W]

    return frames  # Return shape: [T, C, H, W] where T is the actual number of frames
...
if __name__ == "__main__":
    ...
    binary_out, fine_out = model(img)
    
    print("Shape of binary_out :", binary_out.shape)      # [B, 2]
    print("Shape of fine_out :", fine_out.shape)      # [B, num_fine_grained_classes]
    ...
    with torch.no_grad():
        binary_output, fine_output = model(video_tensor)
    
    predicted_binary = torch.argmax(binary_output, dim=1).item()
    predicted_fine = torch.argmax(fine_output, dim=1).item()
    print(f"Predicted binary class: {predicted_binary}")
    print(f"Predicted fine-grained class: {predicted_fine}")
